...
APP_CREDIT = "Created by Mats Romblad, WSP | IDS by ifctester"
...
def estimate_payload(entity: Any, max_list_scan: int = 2000) -> int:
    ...
    for k, v in info.items():
        ...
        if isinstance(v, str):
            ...
            if n and isinstance(v[0], (list, tuple)):
                # deeply nested lists (e.g. points)
    ...
    # Heuristic for heavy geometry arrays not always fully captured by get_info
    try:
...
def classify_entity(entity: Any) -> str:
    """Classifies an entity into broad usage categories."""
    try:
        # Geometry
        if entity.is_a("IfcRepresentationItem"): return "Geometry"
        if entity.is_a("IfcRepresentation"): return "Geometry"
        if entity.is_a("IfcRepresentationContext"): return "Geometry"
        
        # Properties & Metadata
        if entity.is_a("IfcPropertyAbstraction"): return "Properties" # SingleValue, EnumeratedValue, etc.
        if entity.is_a("IfcPropertyDefinition"): return "Properties"  # PropertySet, ElementQuantity
        if entity.is_a("IfcOwnerHistory"): return "Metadata"
        if entity.is_a("IfcRelDefinesByProperties"): return "Properties"
        if entity.is_a("IfcRelAssociates"): return "Metadata"
        
        # Elements (The logical objects)
        if entity.is_a("IfcProduct"): return "Elements"
        
        # Relationships (Structure, Aggregation, Connection - excluding PropDefines)
        if entity.is_a("IfcRelationship"): return "Relationships"
...
def parse_header_info(ifc_file: Any) -> Dict[str, Any]:
    """Extracts all available header information."""
    header_info = {}
    try:
        header_info["SCHEMA"] = getattr(ifc_file, "schema", "Unknown")
        
        # Access wrapped data header
...
def analyze_ifc(filepath: str,
    ...
    # Check for excessive orphans
    if orphan_total > (n_total * 0.2):
        recommendations.append(f"High orphan count detected ({orphan_total:,}). Consider using an IFC optimizer (e.g. Solibri Optimizer or Simplebim) to remove unused definitions.")
    
    # Check for specific heavy types
    if entity_counts["IfcFace"] > 10000 and entity_counts["IfcTriangulatedFaceSet"] < 100:
        recommendations.append("Model uses explicit IfcFace topology heavily. Converting to IfcTriangulatedFaceSet could significantly reduce file size.")
        
    if entity_counts["IfcPolyline"] > 50000:
        recommendations.append("Very high number of IfcPolyline entities. Check if these are 2D annotations or inefficient 3D meshing.")
        
    top_payload_type = payload_by_type.most_common(1)
    if top_payload_type and top_payload_type[0][0] == "IfcPropertySingleValue":
         recommendations.append("Property data is the largest payload. Ensure you are only exporting necessary Property Sets.")
         
    # Check Metadata Ratio
    total_payload = sum(payload_by_type.values())
    prop_payload = payload_by_category.get("Properties", 0) + payload_by_category.get("Metadata", 0)
    if total_payload > 0 and (prop_payload / total_payload) > 0.6:
        recommendations.append("Properties and Metadata account for >60% of file weight. Consider stripping 'Pset_' properties if not needed.")
    ...
    try:
        ...
        if rows:
            with open(csv_geom_path, "w", encoding="utf-8", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=["score", "geometry_type", "geometry_id", "owner_type", "owner_globalid", "owner_name"])
                writer.writeheader()
                for r in rows:
                    writer.writerow(r)
...
def diff_reports(a: Dict[str, Any], b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Compare payloads
    # We need to reconstruct full payload maps if available, but we only stored topN in the simplified return.
    # In a full app we'd pass full payload dicts, but for now we rely on entity count deltas as proxy.
    ...
    return {
        ...
        "by_type_delta_top10": [{"type": k, "delta": v, "a": a["counts"]["full"].get(k,0), "b": b["counts"]["full"].get(k,0)} 
                                for k, v in delta_counts.most_common(10) if v != 0],
        "by_type_delta_bottom10": [{"type": k, "delta": v, "a": a["counts"]["full"].get(k,0), "b": b["counts"]["full"].get(k,0)} 
                                   for k, v in delta_counts.most_common()[:-10:-1] if v != 0]
    }
...
class IfcAnalyzerApp:
    ...
    def render_report(self, report: Dict[str, Any]):
        ...
        if h:
            for k, v in h.items():
                if v: 
                    ...
                    val = str(v)
                    if isinstance(v, (list, tuple)): val = ", ".join(map(str, v))
                    self.log(f"  {k}: {val}")
        else:
        ...
        if not report["geometry"]["heavy_geometry_topN"]:
            ...
            for i, r in enumerate(report["geometry"]["heavy_geometry_topN"]):
                ...
                self.log(f"  #{i+1} [Score {r['score']}] {r['geometry_type']} -> Owner: {owner} ({r['owner_globalid']})")
    ...
    def batch_analyze_folder(self):
        ...
        def work():
            ...
            # Disable deep orphan check for batch to be safe/fast
            opts.deep_orphan_check = False
            ...
            for i, fname in enumerate(files):
                ...
                try:
                    ...
                    line = f"{fname}: {rep['meta']['entity_total']} ents, {rep['payload']['total_proxy_bytes']} load"
                    self.log(line)
    ...
    def compare_two_ifcs(self):
        f1 = filedialog.askopenfilename(title="Select FIRST file (Baseline)", filetypes=[("IFC", "*.ifc")])
        if not f1: return
        f2 = filedialog.askopenfilename(title="Select SECOND file (New)", filetypes=[("IFC", "*.ifc")])
        if not f2: return
        ...
        def work():
            ...
            try:
                ...
                self.log(f"\nEntity Change: {diff['summary']['entity_total']['delta']:+,} (Total: {diff['summary']['entity_total']['b']})")
                self.log("Top Increases (Type):")
                for r in diff["by_type_delta_top10"]:
                    self.log(f"  {r['type']}: {r['delta']:+,} (A:{r['a']} -> B:{r['b']})")
    ...
    def _display_ids_results(self, ids_object):
        ...
        # Iterate specifications in the IDS
        for spec in ids_object.specifications:
            ...
            # For failed specs, try to show details
            if not spec.status:
                self.ids_tree.insert("", "end", values=(status_str, spec.name, "See HTML Report", "Requirements not met"), tags=(status_str,))
            else:
                # Optional: Show passed ones too?
                # self.ids_tree.insert("", "end", values=(status_str, spec.name, "-", "OK"), tags=(status_str,))
                pass
